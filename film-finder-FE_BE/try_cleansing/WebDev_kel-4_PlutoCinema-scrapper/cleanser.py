...
def cleanse_country(movie_data, actor_data):
    ...
    for country in countries:
        id += 1
        temp = {}
        temp['countries_id'] = id
        temp['country_name'] = country
...
def find_actor_id(actor_data, actor_name):
    for actor in actor_data:
        if actor['actor_name'] == actor_name or actor['key'] == actor_name:
...
def create_movie_award(movie_id, awards, award_data):
    ...
    for award in awards:
        ...
        if award_id is not None:
            ...
            current_data.append(temp)
            with open('result_cleanse/film_award.json', 'w') as json_file:
                json.dump(current_data, json_file, indent=4)
        else:
            print('Award not found:', award)
            
def create_movie_actor(movie_id, actors, actor_data):
    current_data = load_json('result_cleanse/film_actor.json')
    for actor in split_string(actors, ','):
        ...
        if actor_id is not None:
            ...
            current_data.append(temp)
            with open('result_cleanse/film_actor.json', 'w') as json_file:
                json.dump(current_data, json_file, indent=4)
        else:
...
def cleanse_movie(movie_data, country_data, actor_data, genre_data, award_data):
    ...
    for movie in movies:
        ...
        temp['url_banner'] = movie['poster']
        temp['title'] = movie['title']
        temp['alternative_title'] = movie['alternative_titles']
        temp['year_release'] = movie['year']
        temp['synopsis'] = movie['synopsis']
        temp['availability'] = movie['availability']
        temp['views'] = movie['views']
        temp['url_trailer'] = movie['trailer']
        temp['rating_film'] = movie['rating']
        temp['status'] = 'accepted'
        temp['duration'] = movie['runtime']
        ...
        for actor in picket_actors:
            new_pick_actors.append(actor['actor_name'])
...
def cleanse_actor(actor_data, country_data):
    ...
    for actor in actors:
        ...
        temp['actor_name'] = actor['name']
        temp['key'] = actor['key']
        temp['url_actor'] = actor['picture_profile']
...
def main():
    ...
    save_to_json(movies, 'result_cleanse/films.json')
